// =============================================================================
// services/aiService.js — VendorBridge AI layer (xAI Grok).
//
// Uses xAI Grok (OpenAI-compatible) when a key is set, else falls back to a
// deterministic local heuristic so every AI feature still works (and demos)
// without any key.
//
// Set in backend/.env:
//   GROK_API_KEY=xai-...                 (your xAI key — also reads XAI_API_KEY)
...
export const aiStatus = () => ({ enabled: provider() !== 'heuristic', provider: provider(), model: provider() === 'grok' ? GROK_MODEL : null });
...
function heuristicRfq(prompt) {
  ...
  return {
    ...
    notes: 'Generated by the local fallback engine. Add a GROK_API_KEY for richer drafts.',
    engine: 'heuristic',
  };
}

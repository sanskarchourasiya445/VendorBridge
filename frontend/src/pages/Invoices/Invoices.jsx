// =============================================================================
// Invoices — payables register with GST split, due dates and payment status.
// =============================================================================
...
export default function Invoices() {
  ...
  const columns = [
    ...
    { key: 'vendorName', header: 'Vendor', render: (i) => <span className="text-slate-700">{i.vendorName}</span> },
    {
      key: 'tax',
      header: 'GST',
      align: 'right',
      render: (i) =>
        i.igst > 0 ? (
          <span className="text-xs text-slate-500">IGST {formatCurrency(i.igst)}</span>
        ) : (
    ...
    },
    {
      ...
      header: 'Total',
      align: 'right',
      render: (i) => <span className="font-semibold text-slate-900">{formatCurrency(i.grandTotal)}</span>,
    },
    {
      key: 'dueDate',
      header: 'Due Date',
      render: (i) => {
        ...
        return (
          <div>
            <p className={overdue ? 'font-medium text-danger-600' : 'text-slate-700'}>{formatDate(i.dueDate)}</p>
            {i.status !== INVOICE_STATUS.PAID && (
              <p className="text-xs text-slate-400">
                {d >= 0 ? `due in ${d} day(s)` : `${Math.abs(d)} day(s) overdue`}
              </p>
            )}
          </div>
        );
    ...
    { key: 'status', header: 'Status', align: 'center', render: (i) => <StatusBadge status={i.status} /> },
  ];

  return (
    <div className="animate-fade-in">
      <PageHeader title="Invoices" subtitle={`${INVOICES.length} invoices on record`} />
